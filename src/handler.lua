...
function ExternalAuthHandler:access(conf)
  ExternalAuthHandler.super.access(self)
    ...
    if response_body["code"] then
            kong.log.err("response codecode: ", table.concat(response_body["code"]))
            if response_body["error_code"] then
                kong.log.err("response error_codeerror_code: ", table.concat(response_body["error_code"]))
            end
    end
    ...
    if response_body["code"] ~= 20101 then
        kong.log.err("response code: ", table.concat(response_body))
        kong.log.err("response error_code1111111111: ", table.concat(response_body["error_code"]))
        kong.log.err("response codecode2222222222222: ", table.concat(response_body["code"]))
        return nil, resp
    end
